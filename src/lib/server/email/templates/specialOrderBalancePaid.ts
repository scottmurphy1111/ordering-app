...
/**
 * Customer-facing email sent when the balance on a deposit-split special order
 * is paid — the order is now fully paid.
 */
export function specialOrderBalancePaidEmail({
	vendorName,
	primaryColor,
    ...
    <div style="margin-top:24px;padding:16px;background:#f0fdf4;border-radius:8px;border-left:4px solid #22c55e;">
      <p style="margin:0 0 4px;font-size:12px;font-weight:600;color:#6b7280;text-transform:uppercase;letter-spacing:0.05em;">Pickup date</p>
      <p style="margin:0;font-size:15px;font-weight:600;color:#111827;">${fmtDate(scheduledFor, vendorTimezone)}</p>
    </div>`
		: '';

	const content = `
    <h1 style="margin:0 0 4px;font-size:24px;font-weight:700;color:#111827;">Balance paid — you're all set!</h1>
    <p style="margin:0 0 24px;font-size:15px;color:#6b7280;">Thanks, ${escapeHtml(customerName)}. We've received the balance on your custom order from ${escapeHtml(vendorName)}. Your order is now paid in full.</p>
    ...
    <div style="margin-bottom:24px;padding:20px;background:#f0fdf4;border-radius:8px;text-align:center;">
      <p style="margin:0 0 4px;font-size:12px;font-weight:600;color:#6b7280;text-transform:uppercase;letter-spacing:0.05em;">Balance paid</p>
      <p style="margin:0;font-size:32px;font-weight:700;color:#111827;">${formatCents(balanceCents)}</p>
      <p style="margin:8px 0 0;font-size:13px;color:#6b7280;">Order total: ${formatCents(totalCents)} — paid in full</p>
    </div>
  ...
  `;
	...
	return emailWrapper({
		title: `Balance paid for order ${orderNumber} — ${vendorName}`,
		previewText: `Your custom order from ${vendorName} is paid in full — ${formatCents(totalCents)}`,
		content,
