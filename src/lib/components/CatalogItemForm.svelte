...
<div>
	<label class="mb-1 block text-sm font-medium text-muted-foreground" for="description">Description</label>
	<textarea
		id="description"
		name="description"
		rows="2"
...
<div class="grid grid-cols-1 gap-3 md:grid-cols-2 md:gap-4">
	...
	<div>
		<label class="mb-1 block text-sm font-medium text-muted-foreground" for="discountedPrice">Sale price ($)</label>
		<input
			id="discountedPrice"
...
{#if mode === 'edit'}
	<Button type="submit" disabled={uploading} variant="default" class={fullWidth ? 'w-full md:w-auto' : ''}>Save changes</Button>
{:else if onSuccess}
	<Button type="submit" disabled={uploading} variant="default" class={fullWidth ? 'w-full md:w-auto' : ''}>Save &amp; add another</Button>
	<Button type="submit" data-close="1" disabled={uploading} variant="outline" class={fullWidth ? 'w-full md:w-auto' : ''}>Save &amp; close</Button>
{:else}
	<Button type="submit" disabled={uploading} variant="default" class={fullWidth ? 'w-full md:w-auto' : ''}>Create item</Button>
	{#if onCancel}
		<Button type="button" onclick={onCancel} variant="outline" class={fullWidth ? 'w-full md:w-auto' : ''}>Cancel</Button>
	{/if}
{/if}
{/snippet}
