<script lang="ts">
	...
	const verifyHref = $derived(
		token ? `/api/auth/magic-link/verify?token=${encodeURIComponent(token)}&callbackURL=${encodeURIComponent(callbackURL)}` : null
	);
</script>
...
<div class="rounded-2xl border border-gray-200 bg-white p-8 shadow-sm text-center">
	{#if !token || !verifyHref}
		...
		<a href="/login" class="mt-4 inline-block text-sm text-green-600 hover:underline">Back to sign in</a>
	{:else}
