...
<div>
	<div class="mb-6 flex items-center justify-between">
		<div>
			<div class="mb-1 flex items-center gap-1.5 text-sm text-muted-foreground">
				<a href={resolve('/dashboard/orders')} class="hover:text-muted-foreground">Orders</a>
				<Icon icon="mdi:chevron-right" class="h-3.5 w-3.5" />
				<span>History</span>
			</div>
			<h1 class="text-2xl font-bold text-foreground">Order History</h1>
		</div>
	...
	<form method="GET" class="mb-5 flex flex-wrap items-center gap-2">
		...
		<div class="relative min-w-0 flex-1">
			...
			<input
				...
				class="w-full rounded-lg border bg-background py-2 pr-4 pl-9 text-sm text-foreground placeholder:text-muted-foreground focus:border-gray-400 focus:outline-none"
				oninput={(e) => debounceSubmit(e.currentTarget.form!)}
			/>
		</div>
		<div class="flex items-center gap-2">
			<label for="date-from" class="text-xs whitespace-nowrap text-muted-foreground">From</label>
			<input
				bind:this={fromInput}
				id="date-from"
				type="date"
				name="from"
				value={data.from}
				class="rounded-md border px-2 py-1.5 text-sm focus:border-gray-400 focus:outline-none"
