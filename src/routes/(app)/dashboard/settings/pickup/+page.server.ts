...
export const actions: Actions = {
	...
	createTemplate: async ({ request, locals }) => {
		try {
			...
			if (error) return fail(400, { error: error });

			let locationId: number | null = null;
			if (locationIdRaw) {
				locationId = parseInt(locationIdRaw);
				if (isNaN(locationId)) return fail(400, { error: 'Invalid location.' });
				...
				if (!loc) return fail(403, { error: 'Location not found.' });
			}
			...
			if (!vendorRecord) return fail(500, { error: 'Vendor record missing.' });
	...
	updateTemplate: async ({ request, locals }) => {
		try {
			...
			const id = parseInt(formData.get('id')?.toString() ?? '');
			if (isNaN(id)) return fail(400, { error: 'Invalid template.' });
			...
			if (!existing) return fail(403, { error: 'Template not found.' });
	...
	toggleTemplateActive: async ({ request, locals }) => {
		try {
			...
			console.error('[toggleTemplateActive] error:', err);
			return fail(500, { error: 'Something went wrong on our end. Please try again.' });
		}
	},
		...
		try {
			...
			if (maxOrdersRaw !== '') {
				...
				if (isNaN(parsed) || parsed < 1)
					return fail(400, { error: 'Capacity must be at least 1.' });
			...
			if (notesRaw.length > 500)
				return fail(400, { error: 'Notes must be 500 characters or fewer.' });
