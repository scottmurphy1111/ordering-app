...
<div>
	<div class="mb-6">
		...
		<p class="text-sm text-gray-500 mt-0.5">Manage who has access to this tenant.</p>
	</div>
	...
	<div class="flex gap-1 mb-6 border-b border-gray-200">
		<button
			...
			class="px-4 py-2 text-sm font-medium border-b-2 transition-colors -mb-px
				{tab === 'members' ? 'border-gray-900 text-gray-900' : 'border-transparent text-gray-500 hover:text-gray-700'}"
		>
			Members
		</button>
		{#if data.canManageInternal}
			<button
				...
				class="px-4 py-2 text-sm font-medium border-b-2 transition-colors -mb-px
					{tab === 'internal' ? 'border-gray-900 text-gray-900' : 'border-transparent text-gray-500 hover:text-gray-700'}"
			>
				Internal users
	...
	{#if tab === 'members'}
		{#if form?.addError}
			<div class="mb-4 rounded-md bg-red-50 border border-red-200 px-4 py-3 text-sm text-red-700">{form.addError}</div>
		{/if}
		{#if form?.error}
			<div class="mb-4 rounded-md bg-red-50 border border-red-200 px-4 py-3 text-sm text-red-700">{form.error}</div>
		{/if}
		{#if form?.addSuccess}
			<div class="mb-4 rounded-md bg-green-50 border border-green-200 px-4 py-3 text-sm text-green-700">Member added.</div>
		{/if}
		{#if form?.inviteError}
			<div class="mb-4 rounded-md bg-red-50 border border-red-200 px-4 py-3 text-sm text-red-700">{form.inviteError}</div>
		{/if}
		...
		{#if data.currentRole === 'owner' || data.currentRole === 'manager'}
			<div class="flex gap-2 mb-5">
				...
				{#if !showInviteForm}
					<button
						onclick={() => { showInviteForm = true; showAddForm = false; }}
						class="rounded-md border border-blue-300 bg-blue-50 px-4 py-2 text-sm text-blue-700 hover:bg-blue-100 transition-colors"
					>
						+ Invite by link
					</button>
				{/if}
			...
			{#if showAddForm}
				<form
					method="post"
					action="?/addMember"
					use:enhance={() => ({ update }) => { update(); showAddForm = false; }}
					class="mb-5 rounded-lg border border-gray-200 bg-white p-4 shadow-sm"
				>
					<h2 class="text-sm font-semibold text-gray-800 mb-3">Add member by email</h2>
					<div class="flex gap-2 flex-wrap">
						<input
							...
							class="flex-1 min-w-48 rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
						/>
						<select
							name="role"
			...
			{#if showInviteForm}
				<form
					method="post"
					action="?/sendInvite"
					use:enhance={() => ({ update }) => { update(); showInviteForm = false; }}
					class="mb-5 rounded-lg border border-blue-200 bg-blue-50 p-4 shadow-sm"
				>
					<h2 class="text-sm font-semibold text-gray-800 mb-1">Invite by link</h2>
					<p class="text-xs text-gray-500 mb-3">Generate a 7-day invite link — works even if they don't have an account yet.</p>
					<div class="flex gap-2 flex-wrap">
						<input
							name="email"
							type="email"
			...
			{#if inviteUrl}
				<div class="mb-5 rounded-lg border border-green-200 bg-green-50 p-4">
					...
					<p class="text-xs text-gray-500 mb-2">Share this link with them — expires in 7 days.</p>
					<div class="flex items-center gap-2">
						...
						<button
							...
							class="shrink-0 rounded-md border border-green-300 bg-white px-3 py-2 text-xs text-green-700 hover:bg-green-100 transition-colors"
						>
					...
					{#each data.members as member (member.userId)}
						<tr class="hover:bg-gray-50 transition-colors">
							<td class="px-4 py-3">
								<p class="font-medium text-gray-900">
									...
									{#if member.isInternal}
										<span class="ml-1 rounded-full bg-indigo-100 px-1.5 py-0.5 text-xs font-medium text-indigo-600">internal</span>
									{/if}
								</p>
							...
							<td class="px-4 py-3">
								...
								{:else}
									<span class="rounded-full px-2 py-0.5 text-xs font-medium {roleColors[member.role] ?? 'bg-gray-100 text-gray-600'}">
		...
		{#if data.pendingInvitations.length > 0}
			<div class="mt-6">
				<h2 class="text-sm font-semibold text-gray-700 mb-3">
					Pending invitations
					<span class="ml-1.5 rounded-full bg-yellow-100 px-1.5 py-0.5 text-xs font-medium text-yellow-700">
	...
	{#if tab === 'internal' && data.canManageInternal}
		<div class="mb-5 rounded-md bg-indigo-50 border border-indigo-200 px-4 py-3 text-sm text-indigo-700">
			Internal users are platform-level staff with access to all tenants. This is separate from tenant membership roles.
		</div>

		{#if data.internalUsers.length === 0}
			<div class="rounded-xl border border-dashed border-gray-300 p-10 text-center mb-6">
				<p class="text-gray-400 text-sm">No internal users yet.</p>
		...
		{#if nonInternalMembers.length > 0}
			<div>
				<h2 class="text-sm font-semibold text-gray-700 mb-3">Promote a current member to internal</h2>
