	...
	<div class="space-y-6">
				...
				<form method="post" action="?/saveColors" use:enhance class="space-y-5">
					<div class="grid grid-cols-1 gap-6 sm:grid-cols-3">
						{#each [
							...
							{ id: 'secondaryColor', label: 'Secondary color', bind: 'secondaryColor', note: 'Category pills, badges, tinted sections.', value: secondaryColor },
							{ id: 'accentColor', label: 'Accent color', bind: 'accentColor', note: 'Text on primary backgrounds.', value: accentColor }
						] as col (col.id)}
							<div>
								<label class="block text-sm font-medium text-gray-700 mb-2" for={col.id}>{col.label}</label>
								<div class="flex items-center gap-3">
									<div class="relative h-10 w-10 shrink-0 overflow-hidden rounded-md border border-gray-300 shadow-sm">
										<input
											...
											id={col.id}
											name={col.id}
											value={col.id === 'primaryColor' ? primaryColor : col.id === 'secondaryColor' ? secondaryColor : accentColor}
											oninput={(e) => {
										...
										/>
										<div class="h-full w-full rounded-md" style="background-color: {col.id === 'primaryColor' ? primaryColor : col.id === 'secondaryColor' ? secondaryColor : accentColor};"></div>
									</div>
									<input
										type="text"
										value={col.id === 'primaryColor' ? primaryColor : col.id === 'secondaryColor' ? secondaryColor : accentColor}
					...
					<!-- Live preview -->
					<div>
						<p class="mb-2 text-xs font-medium uppercase tracking-wide text-gray-500">Preview</p>
						<div class="overflow-hidden rounded-lg border border-gray-200">
							...
							<div class="flex items-center gap-3 px-4 py-4" style="background-color: {accentColor};">
								...
								<span class="rounded-full border px-3 py-1 text-xs font-medium" style="border-color: {secondaryColor}; color: {secondaryColor};">Drinks</span>
								<button type="button" class="ml-auto rounded-md px-3 py-1.5 text-xs font-semibold shadow-sm" style="background-color: {primaryColor}; color: {accentColor};">+ Add</button>
							</div>
						</div>
					</div>
		...
		<div class="rounded-xl border border-gray-200 bg-white shadow-sm">
			<div class="border-b border-gray-100 px-5 py-4 flex items-center justify-between">
				...
				<span class="rounded-full px-2.5 py-0.5 text-xs font-medium {data.branding.logoUrl ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}">
			...
			<div class="space-y-4 px-5 py-5">
				{#if data.branding.logoUrl}
					<div class="flex items-center gap-4 rounded-lg border border-gray-200 bg-gray-50 p-4">
						<img src={data.branding.logoUrl} alt="Logo" class="h-16 w-auto max-w-40 object-contain" />
					</div>
				{/if}
				{#if logoState.error}
				...
				{/if}
				<div class="flex items-center gap-3">
					<input type="file" accept="image/jpeg,image/png,image/webp,image/svg+xml" bind:this={logoInput}
						onchange={(e) => { const f = (e.target as HTMLInputElement).files?.[0]; if (f) uploadImage(f, '/api/upload-logo', 'logo', logoState, 2); }} class="hidden" />
					<button type="button" onclick={() => logoInput?.click()} disabled={logoState.uploading}
						...
						{logoState.uploading ? 'Uploading…' : data.branding.logoUrl ? 'Replace logo' : 'Upload logo'}
					</button>
					{#if data.branding.logoUrl}
						<form method="post" action="?/removeLogo" use:enhance>
							<button type="submit" class="rounded-md border border-red-200 px-4 py-2 text-sm text-red-600 hover:bg-red-50 transition-colors">Remove</button>
						</form>
					{/if}
				</div>
				<p class="text-xs text-gray-400">JPG, PNG, WebP, or SVG · max 2MB · transparent PNG or SVG recommended</p>
			</div>
		</div>
		...
		<div class="rounded-xl border border-gray-200 bg-white shadow-sm">
			<div class="border-b border-gray-100 px-5 py-4 flex items-center justify-between">
				<div>
					...
					<p class="mt-0.5 text-xs text-gray-500">Full-bleed hero image shown at the top of your menu page.</p>
				</div>
				<span class="rounded-full px-2.5 py-0.5 text-xs font-medium {data.branding.bannerUrl ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}">
			...
			<div class="space-y-4 px-5 py-5">
				...
				{/if}
				<div class="flex items-center gap-3">
					<input type="file" accept="image/jpeg,image/png,image/webp" bind:this={bannerInput}
						onchange={(e) => { const f = (e.target as HTMLInputElement).files?.[0]; if (f) uploadImage(f, '/api/upload-banner', 'banner', bannerState, 5); }} class="hidden" />
					<button type="button" onclick={() => bannerInput?.click()} disabled={bannerState.uploading}
						class="rounded-md bg-gray-900 px-4 py-2 text-sm font-medium text-white hover:bg-gray-700 transition-colors disabled:opacity-50">
						{bannerState.uploading ? 'Uploading…' : data.branding.bannerUrl ? 'Replace banner' : 'Upload banner'}
					</button>
					{#if data.branding.bannerUrl}
				...
				<p class="text-xs text-gray-400">JPG, PNG, or WebP · max 5MB · wide landscape image recommended (1600×600px or similar)</p>
			</div>
		</div>
		...
		<div class="rounded-xl border border-gray-200 bg-white shadow-sm">
			<div class="border-b border-gray-100 px-5 py-4 flex items-center justify-between">
				<div>
					...
					<p class="mt-0.5 text-xs text-gray-500">Subtle full-page texture behind your menu and cart. Works best with low-contrast images (wood, marble, linen).</p>
				</div>
				<span class="rounded-full px-2.5 py-0.5 text-xs font-medium {data.branding.backgroundImageUrl ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}">
			...
			<div class="space-y-4 px-5 py-5">
				{#if data.branding.backgroundImageUrl}
					<div class="overflow-hidden rounded-lg border border-gray-200">
						<img src={data.branding.backgroundImageUrl} alt="Background" class="h-28 w-full object-cover opacity-60" />
					</div>
				{/if}
				{#if bgState.error}
				...
				{/if}
				<div class="flex items-center gap-3">
					<input type="file" accept="image/jpeg,image/png,image/webp" bind:this={bgInput}
						onchange={(e) => { const f = (e.target as HTMLInputElement).files?.[0]; if (f) uploadImage(f, '/api/upload-background-image', 'backgroundImage', bgState, 5); }} class="hidden" />
					<button type="button" onclick={() => bgInput?.click()} disabled={bgState.uploading}
						class="rounded-md bg-gray-900 px-4 py-2 text-sm font-medium text-white hover:bg-gray-700 transition-colors disabled:opacity-50">
						{bgState.uploading ? 'Uploading…' : data.branding.backgroundImageUrl ? 'Replace background' : 'Upload background'}
					</button>
					{#if data.branding.backgroundImageUrl}
				...
				<p class="text-xs text-gray-400">JPG, PNG, or WebP · max 5MB · tileable textures work best</p>
			</div>
		</div>
