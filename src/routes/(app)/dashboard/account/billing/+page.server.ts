...
export const actions: Actions = {
	upgrade: async ({ request, locals }) => {
		try {
			...
			if (record?.subscriptionTier === planKey)
				return fail(400, { error: 'Already on this plan.' });
			const stripe = getOrderLocalStripe();
			...
			if (record?.stripeSubscriptionId) {
				...
				try {
					...
					console.error(
						'[upgrade] subscription retrieve failed, treating as no subscription:',
						err
					);
				}
	...
	switchInterval: async ({ request, locals }) => {
		try {
			...
			console.error('[switchInterval] error:', err);
			return fail(500, { error: 'Something went wrong on our end. Please try again.' });
		}
	},
	...
	cancelImmediate: async ({ locals }) => {
		try {
			...
			console.error('[cancelImmediate] error:', err);
			return fail(500, { error: 'Something went wrong on our end. Please try again.' });
		}
	},
	...
	activateAddon: async ({ request, locals }) => {
		try {
			...
			console.error('[activateAddon] error:', err);
			return fail(500, { error: 'Something went wrong on our end. Please try again.' });
		}
	},
	...
	deactivateAddon: async ({ request, locals }) => {
		try {
			...
			console.error('[deactivateAddon] error:', err);
			return fail(500, { error: 'Something went wrong on our end. Please try again.' });
		}
	},
	...
	pauseSubscription: async ({ request, locals }) => {
		try {
			...
			if (record.subscriptionPausedAt)
				return fail(400, { error: 'Subscription is already paused.' });
	...
	refundAccountCredit: async ({ locals }) => {
		try {
			...
			console.error('[refundAccountCredit] error:', err);
			return fail(500, { error: 'Something went wrong on our end. Please try again.' });
		}
	}
};
