<script lang="ts">
	...
	let fulfillmentModelValue = $state<string>('');
		...
		<Card class="shadow-sm">
			...
			<CardContent>
				<form method="post" action="?/create" use:enhance class="space-y-4">
					...
					<div>
						<p class="mb-1 block text-sm font-medium text-muted-foreground">Fulfillment model</p>
						<p class="mb-2 text-xs text-muted-foreground">
							How will customers receive their orders?
						</p>
						<div class="space-y-2">
							{#each FULFILLMENT_MODELS as model (model.value)}
								<label
									class="flex cursor-pointer items-start gap-3 rounded-lg border bg-background p-3 transition-colors hover:bg-muted/40 {fulfillmentModelValue ===
									model.value
								...
								>
									<input
										...
										value={model.value}
										bind:group={fulfillmentModelValue}
										class="mt-0.5"
										required
									/>
									<div class="flex-1">
										<p class="text-sm font-medium text-foreground">{model.label}</p>
