<script lang="ts">
	...
	onMount(() => {
		...
		const interval = setInterval(() => { openStatus = getOpenStatus(hours); }, 60_000);
	...
	function matchesQuery(item: { name: string; description?: string | null }, q: string) {
		...
		return item.name.toLowerCase().includes(lq) || (item.description?.toLowerCase().includes(lq) ?? false);
	}

	const filteredCategorized = $derived(
		searchQuery.trim()
			? categorized
				.map((cat) => ({ ...cat, items: cat.items.filter((i: (typeof cat.items)[number]) => matchesQuery(i, searchQuery)) }))
	...
	function addSimple(item: {
		...
		pulsingId = item.id;
		setTimeout(() => { pulsingId = null; }, 350);

		if (addedTimer) clearTimeout(addedTimer);
		lastAddedId = item.id;
		addedTimer = setTimeout(() => { lastAddedId = null; }, 2500);
	}
...
<div class="min-h-screen">
	...
	{#if tenant.bannerUrl}
		<div class="relative h-56 overflow-hidden sm:h-72">
			...
			<div class="absolute inset-x-0 bottom-0 mx-auto max-w-2xl px-4 pb-5">
					...
					<div class="flex flex-wrap items-center gap-2">
						{#if tenant.website}
							<a
								...
								class="text-2xl font-bold text-white drop-shadow hover:underline"
							>{tenant.name}</a>
						{:else}
				...
				<div class="flex flex-wrap items-center gap-2">
					{#if openStatus !== null}
						<span class="rounded-full px-2.5 py-0.5 text-xs font-semibold {openStatus ? 'bg-green-500 text-white' : 'bg-red-500 text-white'}">
							{openStatus ? 'Open' : 'Closed'}
						</span>
					{/if}
					{#if tableParam}
						<span class="flex items-center gap-1 rounded-full bg-white/20 px-2.5 py-0.5 text-xs font-semibold text-white backdrop-blur-sm">
		...
		<header style="background-color: var(--primary-color);">
			<div class="mx-auto max-w-2xl px-4 py-5">
				<div class="flex items-center gap-4">
					{#if tenant.logoUrl}
						<img src={tenant.logoUrl} alt={tenant.name} class="h-16 w-auto max-w-48 shrink-0 object-contain" />
					{/if}
					<div>
						{#if tenant.website}
							<a href={tenant.website} target="_blank" rel="noopener noreferrer" class="hover:opacity-80 transition-opacity">
				...
				<div class="mt-2 flex flex-wrap items-center gap-2">
					{#if tenant.type}
						<p class="text-sm capitalize opacity-75" style="color: var(--accent-color);">{tenant.type.replace('_', ' ')}</p>
					{/if}
					{#if openStatus !== null}
						<span class="rounded-full px-2.5 py-0.5 text-xs font-semibold {openStatus ? 'bg-green-500 text-white' : 'bg-red-500/90 text-white'}">
							{openStatus ? 'Open' : 'Closed'}
						</span>
					{/if}
					{#if tableParam}
						<span class="flex items-center gap-1 rounded-full bg-white/20 px-2.5 py-0.5 text-xs font-semibold backdrop-blur-sm" style="color: var(--accent-color);">
							<Icon icon="mdi:table-chair" class="h-3 w-3" /> Table {tableParam}
						</span>
					{/if}
				</div>
		...
		<div class="sticky top-0 z-50 border-b border-gray-200 bg-white/95 backdrop-blur-sm">
			<div class="mx-auto max-w-2xl px-4 pt-2">
				...
				<div class="relative mb-2">
					<Icon icon="mdi:magnify" class="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400" />
					<input
						type="search"
						placeholder="Search menu…"
						bind:value={searchQuery}
						class="w-full rounded-full border border-gray-200 bg-gray-50 py-2 pl-9 pr-4 text-sm outline-none transition-colors focus:border-gray-400 focus:bg-white"
					/>
					{#if searchQuery}
						<button
							onclick={() => { searchQuery = ''; }}
					...
					<div class="overflow-x-auto">
						<div class="flex gap-1 pb-2">
							{#each filteredCategorized as category (category.id)}
								<a
									href="#{category.id}"
									class="category-pill shrink-0 rounded-full px-4 py-1.5 text-sm font-medium text-gray-600 transition-colors {activeCategoryId === String(category.id) ? 'active' : ''}"
								>
									{category.name}
								</a>
							{/each}
							{#if filteredUncategorized.length > 0}
								<a
									href="#other"
									class="category-pill shrink-0 rounded-full px-4 py-1.5 text-sm font-medium text-gray-600 transition-colors {activeCategoryId === 'other' ? 'active' : ''}"
								>
									Other
								</a>
							{/if}
	...
	<!-- ── Menu items ─────────────────────────────────────────────────────── -->
	<main class="mx-auto max-w-2xl space-y-10 rounded-2xl bg-white/80 px-4 py-8 backdrop-blur-sm">
		{#if data.items.length === 0}
		...
		{:else if !hasResults}
			<div class="rounded-xl border border-dashed border-gray-300 p-12 text-center">
				...
				<button onclick={() => { searchQuery = ''; }} class="mt-3 text-sm font-medium" style="color: var(--primary-color);">Clear search</button>
			</div>
		{:else if !mounted}
			<!-- Skeleton -->
			<div class="space-y-3">
				...
				<section id={String(category.id)}>
					...
					<div class="space-y-3">
						{#each category.items as item (item.id)}
							...
							<div class="item-card flex gap-4 rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
								{#if primaryImage}
									<img src={primaryImage} alt={item.name} class="h-20 w-20 shrink-0 rounded-lg object-cover" />
								{/if}
									...
									{#if Array.isArray(item.tags) && item.tags.length > 0}
										<div class="mt-1.5 flex flex-wrap gap-1">
											{#each item.tags as tag (tag)}
												<span class="rounded-full px-2 py-0.5 text-xs" style="background-color: color-mix(in srgb, var(--secondary-color) 15%, white); color: var(--secondary-color);">{tag}</span>
											{/each}
										</div>
									{/if}
								</div>
								<div class="flex shrink-0 flex-col items-end justify-between gap-2">
									<div class="text-right">
										{#if item.discountedPrice}
											<p class="font-semibold" style="color: var(--primary-color);">${(item.discountedPrice / 100).toFixed(2)}</p>
									...
									{:else}
										<button
											...
											class="add-btn rounded-lg px-3 py-1.5 text-xs font-medium transition-all {pulsingId === item.id ? 'pulsing' : ''} {lastAddedId === item.id ? 'added' : ''}"
											style={lastAddedId === item.id ? '' : 'background-color: var(--primary-color); color: var(--accent-color);'}
										>
			...
			{#if filteredUncategorized.length > 0}
				<section id="other">
					{#if filteredCategorized.length > 0}
						<h2 class="mb-4 border-b-2 pb-2 text-lg font-semibold text-gray-800" style="border-color: var(--primary-color);">Other</h2>
					{/if}
					<div class="space-y-3">
	...
	{#if cart.count > 0}
		<div class="sticky bottom-0 flex justify-center p-4">
			...
			>
				<span class="flex h-6 w-6 items-center justify-center rounded-full bg-white/20 text-sm font-bold">{cart.count}</span>
