...
### Feedback (Alerts and Toasts)

- **Save form feedback** — convention for form-action UX: in-flight → spinner-on-button, success → `toast.success(...)`, error → inline `<Alert severity="error">`. Page-load info banners stay inline. → [Save form feedback](#save-form-feedback)
- **Alert Pattern A — page-load banner / result panel** — narrow surviving use: query-param-driven banner after a redirect, OR structured multi-line result panel. NOT for form-action success — use Save form feedback instead. → [Alert (severity-based feedback)](#alert-severity-based-feedback)
- **Alert Pattern B — persistent state banner** — describes an ongoing condition rather than a one-shot event. `dismissible={false}`, `autofade={0}`. → [Alert (severity-based feedback)](#alert-severity-based-feedback)
...
```ts
const recurrence = templateKind === 'weekly' ? `FREQ=WEEKLY;BYDAY=${days.join(',')}` : 'FREQ=DAILY';
...
**Pattern A — Page-load banner / result panel (narrow surviving use):**

Pattern A was historically the dominant pattern for form-action success and error feedback (`{#if form?.someSuccess}<Alert severity="success">`). That role was migrated to the [Save form feedback](#save-form-feedback) convention — success → `toast.success(...)`, error → inline `<Alert severity="error">`. **Do not use Pattern A for form-action success feedback.** Use the toast convention instead.

Two narrow surviving cases for `<Alert severity="success">` exist:

**1. Page-load banner from a query-param redirect.** A server action redirects to a page with a flag (e.g. `?fulfillmentChanged=1`); the destination page reads the flag from `data.*` and renders a banner. This is not form-action feedback (the user arrived via redirect, not via a form result on this page).

```svelte
{#if data.fulfillmentChanged}
	<Alert severity="success">Fulfillment model updated.</Alert>
{/if}
```

For Stripe-return redirects (`?upgraded=1`, `?paused=1`, etc.), prefer the **mount-time toast pattern** instead — fire `toast.success(...)` from `onMount` and clean the URL with `replaceState`. See `account/billing/+page.svelte` for the canonical example. Pattern A's inline Alert form is reserved for cases where the banner is intended to persist on the page (the fulfillment-model banner is one), not just confirm an action and disappear.

**2. Structured result panel from a non-form fetch flow.** A multi-line panel that surfaces inline after an action whose result has structure beyond a single confirmation message (e.g. import counts: "5 items imported, 2 skipped"). Toasts can't render structured multi-line content well; an inline Alert is correct.
...
`catalog/items/+page.svelte` is the canonical site.

**For any form-action feedback that doesn't fit one of these two narrow cases, use the [Save form feedback](#save-form-feedback) convention.** That's success → toast, error → inline `<Alert severity="error">`, in-flight → spinner-on-button.

Severity choice for the surviving Pattern A cases follows the action's semantic outcome:

- Action completed cleanly (page-load banner, structured result): `severity="success"`
- Server or validation error in a non-form context: `severity="error"`
- State change without celebration or alarm: `severity="info"`
- Needs attention, not blocking: `severity="warning"`

Note: `severity="success"` covers both creation and destruction confirmations. The severity signals the action completed cleanly, not that something was added.

**Pattern B — Persistent state banner / informational context:**
...
Use for Alerts that describe ongoing state rather than just-completed events. `dismissible={false}` prevents dismissal (the message describes a current condition that's still true). `autofade={0}` disables auto-fade regardless of severity default. The Alert persists as long as the underlying state holds.

Canonical sites: `account/billing/+page.svelte` (paused, past-due, cancel-scheduled banners + persistent info banners inside plan-change cards). The distinction from Pattern A is _what the message describes_ — a one-shot event (Pattern A: "Fulfillment model updated") versus an ongoing condition (Pattern B: "Your subscription is paused"). Pattern A appears once after the action and may scroll off; Pattern B remains visible as long as the state is true.
...
When the shift happens _below the fold_, the visible symptom is NOT
...
```ts
type OpenState = { isOpen: true; closesAt: Date } | { isOpen: false; opensAt: Date | null }; // opensAt=null means closed for the rest of the day
```
